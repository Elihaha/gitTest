...
import com.bst.common.pojo.ExpressTypes;
...
//        Query query = new Query(params);
            ...
            //   修改或者初始化某些参数
            updateOrInitParam(params, status, stringObjectHashMap);
            ...
            result.put("data", orderLogisticsList);
            result.put("sum", total);
            return Result.ok(result);
    ...
    private void updateOrInitParam(@ApiParam OrderLogisticscQueryConditions params, Integer status, HashMap<String, Object> stringObjectHashMap) {
        // 商户id
        long shopId = PermissionInfoUtil.getCurrentLogginUser().getShopInfo().getId();
        stringObjectHashMap.put("shopId", shopId);
//            状态
...
//            List<ExpressType> collect = Stream.of(ExpressType.values()).collect(Collectors.toList());
    ...
    public Result upload(MultipartFile file) throws IOException {
        ...
        try {
            ...
            if(defaultSystemUploadPath instanceof LinuxUploadPath){
                Runtime.getRuntime().exec("chmod 777 " + defaultSystemUploadPath.getCurrentSystemUploadPath());
            }
            //创建本地文件夹
            File file1 = new File(defaultSystemUploadPath.getCurrentSystemUploadPath());
            file1.mkdirs();
            //创建本地文件
...
//            trackingNumber = 889999834560284183L+"";
            ...
            if (!queryKuaidiJson.isPresent()) {
                return Result.error(HttpConstants.SUCCESS, "没有任何数据");
            }
            QueryKuaidiJson result = queryKuaidiJson.get();
