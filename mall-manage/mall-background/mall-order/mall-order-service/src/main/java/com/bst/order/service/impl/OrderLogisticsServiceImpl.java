	...
	public List<OrderLogisticsVO> queryList(Pagination pagination, Map<String, Object> like){
        ...
        List<OrderLogisticsVO> collect = orderLogisticsDtos.stream()
				.map(orderLogisticsDto ->{
							OrderLogisticsVO orderLogisticsVO = orderLogisticsDto.castOrderLogisticsPojo(1);
							orderLogisticsVO.setDhl(jedisCluster.get(redisParam.getCourierCompanyCode()+orderLogisticsVO.getDhl()));
