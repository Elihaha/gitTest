...
public class RedisClient {
    ...
    private JedisCluster initJedisCluster() {
        ...
        if(StringUtils.isBlank(password)) {
            ...
            jedisCluster = new JedisCluster(jedisClusterNodes,300,300,5, password,config);
        }
        nodeMap = jedisCluster.getClusterNodes();
        String anyHost = nodeMap.keySet().iterator().next();
        slotHostMap = getSlotHostMap(anyHost, password);
        return jedisCluster;
    }
    ...
    private static TreeMap<Long,String> getSlotHostMap(String anyHostAndPortStr, String password) {
